...
local function safeUtf8(str)
    ...
    if valid then
        ...
        debugLog(string.format("[DEBUG] ⚠️  UTF-8 오류 감지, 안전 모드로 변환: %s", str:sub(1, 20)))
...
local function closeDebugLog()
    if debugLogFile then
        ...
        if debugLogPath then
            print(string.format("\n[INFO] 디버그 로그가 저장되었습니다: %s", debugLogPath))
        end
    end
end
...
local function importFromPsd(filename)
    ...
    for i = 1, layerCount do
        ...
        ----------------------------------------------------------------
        -- 🔄  추가 정보 영역 전체 스캔  (기존 single-if → while-loop)
        ----------------------------------------------------------------
        local curPos       = file:seek("cur")
        local processed    = curPos - extraStart
        local remaining    = extraLength - processed

        while remaining >= 12 do                        -- header 12 B 보다 크면 반복
            local addSig  = file:read(4)                  -- "8BIM"
            local addKey  = file:read(4)                  -- "luni" / "lsct" …
            local addLen  = readU32BE(file)               -- payload 길이
            local padded  = addLen + (addLen % 2)         -- 2-byte even 패딩
            remaining = remaining - (12 + padded)

            if addSig ~= "8BIM" then                      -- 예상치 못한 시그니처
                file:seek("cur", -8)                        -- 되돌리고 나머지 skip
                if remaining > 0 then file:read(remaining) end
                break
            end

            if addKey == "lsct" and addLen >= 4 then      -- ★ 폴더 정보 발견
                layer.groupType = readU32BE(file)           -- 0/1/2=오프너, 3=클로저
                layer.isGroup   = true
                if padded > 4 then                          -- 나머지 skip
                    file:read(padded - 4)
                end
            elseif addKey == "luni" and addLen >= 4 then   -- ★ Unicode 레이어 이름 발견
                local count = readU32BE(file)               -- UTF-16 code unit 수
                local bytesToRead = count * 2
                if bytesToRead > 0 and bytesToRead <= addLen - 4 then
                    local utf16 = file:read(bytesToRead)    -- UTF-16BE 데이터 읽기
                    layer.name = utf16beToUtf8(utf16)       -- UTF-8로 변환
                    debugLog(string.format("[DEBUG]   ✅ Unicode 이름 변환: %s", layer.name))
                    if padded > 4 + bytesToRead then        -- 나머지 패딩 skip
    ...
    -- 🔍 디버그: 시작 메시지
    debugLog(string.format("\n[DEBUG] === PSD 레이어 처리 시작 (총 %d개) ===", layerCount))
    ...
    for i = layerCount, 1, -1 do
        ...
        if layerInfo.isGroup then
            ----------------------------------------------------------
            -- A. 오프너: type 0·1·2  →  폴더를 만들고 **continue**
            ----------------------------------------------------------
            if layerInfo.groupType == 0 or layerInfo.groupType == 1
               or layerInfo.groupType == 2 then

                debugLog(string.format("[DEBUG] → 그룹 오프너 생성: %s (Type %d)", 
                ...
                if #groupStack > 0 then
                    ...
                    debugLog(string.format("[DEBUG]   부모 그룹: %s", groupStack[#groupStack].name))
                end
                grp.stackIndex = 1            -- 맨 아래로
                table.insert(groupStack, grp) -- push
                
                debugLog(string.format("[DEBUG]   그룹 생성 완료, 스택 크기: %d", #groupStack))

                ------------------------- 중요 -------------------------
                goto nextRecord              -- **여기서 끝내야 이중생성 방지**
                --------------------------------------------------------
            ----------------------------------------------------------
            -- B. 클로저: type 3  →  폴더 닫고 **continue** (레이어 x)
            ----------------------------------------------------------
            elseif layerInfo.groupType == 3 then
                debugLog(string.format("[DEBUG] → 그룹 클로저: %s (Type 3)", layerInfo.name))
                if #groupStack > 0 then 
                    local closedGroup = table.remove(groupStack)
                    debugLog(string.format("[DEBUG]   그룹 닫음: %s, 스택 크기: %d", 
        ...
        ------------------------------------------------------------
        -- (2) 일반 레이어 처리 (isGroup == false) -----------------
        ------------------------------------------------------------
        debugLog(string.format("[DEBUG] → 일반 레이어 생성: %s", layerInfo.name))
    ...
    for i, layer in ipairs(sprite.layers) do
        if layer.isGroup then
            debugLog(string.format("[DEBUG] %d: [폴더] %s (isExpanded=%s)", 
                i, layer.name, tostring(layer.isExpanded)))
        else
            debugLog(string.format("[DEBUG] %d: [레이어] %s", i, layer.name))
        end
    end
    debugLog("[DEBUG] ========================\n")
...
local function showImportDialog()
    ...
    dialog:file {
        ...
        text = "PSD → Aseprite 가져오기 도구\n(RGB/RGBA 8bit + 그룹 구조 지원)"
    }
    ...
    if dialog.data.ok then
        ...
        if filename and filename ~= "" then
            ...
            if success then
                app.alert({
                    title = "가져오기 완료",
                    text = "PSD 파일을 성공적으로 가져왔습니다!\n" .. filename,
                    buttons = "OK"
                })
            else
                app.alert({
                    title = "가져오기 실패",
                    text = "PSD 파일을 가져오는 중 오류가 발생했습니다:\n" .. (errorMessage or "알 수 없는 오류"),
                    buttons = "OK"
                })
            end
        end
